...
def test_concatenacao_com_dataframe_vazio():
    ...
    assert len(result) == len(
        df1
    )  # O resultado deve ter o mesmo número de linhas que df1
    pd.testing.assert_frame_equal(
        result, df1
    )  # O resultado deve ser igual a df1
...
def test_concatenacao_com_colunas_diferentes():
    ...
    )  # Deve haver valores NaN devido às colunas diferentes
